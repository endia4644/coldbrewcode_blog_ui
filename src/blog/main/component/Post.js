      ...
      observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
          if (entry.isIntersecting && !isFetching) {
            dispatch(actions.fetchAllPost(post, totalCount, hashtagCurrent, searchCurrent));
          }
        })
      })
          ...
          <>
            <List.Item
              ...
              actions={[
                ...
                <IconText icon={HeartOutlined} text={item.likeCnt} key="list-vertical-like-o" />,
                <IconText icon={MessageOutlined} text={item.Comments?.length ?? 0} key="list-vertical-message" />,
              ...
              {item.postContent}
              {
                ...
                <Col>
                  {item.Hashtags.map((item, i) => (
                    <Button key={`button_${i}`} className="tag-button" type="primary" shape="round" style={{ marginTop: 10, marginRight: 10 }}>{item.hashtagName}</Button>
                  ))}
                </Col>
              }
            </List.Item>
