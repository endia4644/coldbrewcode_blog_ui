...
import { API_HOST, AuthStatus, FetchStatus } from "../constant";
import useFetchInfo from "../hook/useFetchInfo";
import { Types as authTypes } from "../../blog/auth/state"
import { actions as mainActions, Types as mainTypes } from "../../blog/main/state";
import { actions as commonActions } from "../state";
/**
 *
 * @param {object} param
...
export default function Settings({ logout }) {
   ...
   */
  const onClickSearch = () => {
    dispatch(commonActions.setValue({key: "searchFlag", value: true}));
  }
  ...
  useEffect(() => {
    ...
    */
    if (outFetchStatus === FetchStatus.Success) {
      dispatch(mainActions.setValue({key: 'post', value: []}));
  ...
  function menuClick({ key }) {
    switch (key) {
      case "1":
        ...
        break;
      case "2":
        navigate(`/blog/@${user.nickName}/temp/post`)
        break;
      case "3":
        navigate(`/blog/@${user.nickName}/like/post`)
        break;
      case "4":
